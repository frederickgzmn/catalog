...
- ✅ **Apollo Client** integration with custom GraphQLSSELink
- ✅ **Vanilla JavaScript** support via standard fetch API
- ✅ **GraphQL-SSE Client** library compatibility
- 🚧 **Urql, Relay** support planned via custom adapters
...
│   + SSE Link    │                     │   SSE-2         │
│                 │ ←─────────────────  │   Sidecar       │
│   useSubscription│    SSE Stream      │                 │
└─────────────────┘                     └─────────────────┘
                                                │
┌─────────────────┐    Subscription     │ Redis Events
│  Vanilla JS     │ ─────────────────→  ▼
│  + Fetch API    │                     ┌─────────────────┐
│                 │ ←─────────────────  │                 │
│  ReadableStream │    SSE Stream      │   Redis         │
└─────────────────┘                     │   Pub/Sub       │
                                        │                 │
┌─────────────────┐    Subscription     └─────────────────┘
│  GraphQL-SSE    │ ─────────────────→          ▲
│  Client Lib     │                             │ Events
│                 │ ←─────────────────          │
│  createClient() │    SSE Stream      ┌─────────────────┐    Execute Document    ┌─────────────────┐
└─────────────────┘                    │                 │ ──────────────────→   │                 │
                                       │   WordPress     │                       │   WPGraphQL     │
                                       │   Events        │ ←────────────────────  │   Endpoint      │
                                       │                 │    Response            │                 │
...
- **Apollo Client**: Custom `GraphQLSSELink` with `useSubscription` hook support
- **Vanilla JavaScript**: Direct fetch API integration with `ReadableStream`
- **GraphQL-SSE Client**: Official client library compatibility
- **Production Examples**: Complete React + TypeScript demos with Vite
