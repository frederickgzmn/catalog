...
export class ChannelBuilder {
  ...
  /**
   * Build single optimal channel for subscription (prevents duplicates)
   * Returns specific channel if arguments exist, otherwise global channel
   */
  static buildOptimal(subscriptionName: string, args: Record<string, any> = {}): string {
    // If arguments exist, use specific channel only
    if (args && Object.keys(args).length > 0) {
      return this.build(subscriptionName, args);
    }
    
    // No arguments - use global channel
