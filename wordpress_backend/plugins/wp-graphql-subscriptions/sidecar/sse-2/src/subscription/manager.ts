...
import { SubscriptionExecutionCache, type ExecutionResult } from './execution-cache.js';
...
export class SubscriptionManager {
  ...
  private executionCache: SubscriptionExecutionCache;
  ...
  async createSubscription(
    ...
    // Build Redis channel name - use optimal single channel to prevent duplicates
    const channel = ChannelBuilder.buildOptimal(subscriptionName, args);
  ...
  private async sendEventToSubscription(subscription: ActiveSubscription, payload: any): Promise<void> {
    try {
      ...
      // Only send the event if there's actual data (not null)
      // This allows GraphQL resolvers to filter events based on user permissions
      const subscriptionData = result.data?.[subscription.subscriptionName];
      ...
      if (subscriptionData !== null && subscriptionData !== undefined) {
        ...
        // Resolver returned null - user doesn't have permission or data doesn't match filter
        this.logger.debug({ 
          subscriptionId: subscription.id,
          subscriptionName: subscription.subscriptionName,
          hasData: !!result.data,
          hasErrors: !!result.errors,
          subscriptionData
        }, 'Resolver returned null - not sending event to client (permission/filter)');
      }
